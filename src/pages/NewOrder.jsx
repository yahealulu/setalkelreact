...
const OrderProductSelection = ({ onOrderComplete }) => {
    ...
    const { data: categories, isLoading, error } = useQuery({
        ...
        queryFn: async () => {
            const { data } = await axios.get(`${process.env.REACT_APP_API_URL || 'https://setalkel.amjadshbib.com/api/'}countries/${orderData.country_id}/categories`)
            return data?.data
        },
    })
...
export default function NewOrder() {
    ...
    const { data: countries } = useQuery({
        queryKey: ['countries'],
        queryFn: async () => {
            const { data } = await axios.get(`${process.env.REACT_APP_API_URL || 'https://setalkel.amjadshbib.com/api/'}countries`)
            return data?.data
        }
    })
